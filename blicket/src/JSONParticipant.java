...
public class JSONParticipant {
    ...
    public JSONParticipant(JsonObject j) {
        ...
        this.inclusion = (this.attention1 == "1" && this.attention2 == "1" && this.shape.toLowerCase() == "shape");
    ...
    public JsonObject forcedChoice(){
        ...
        b.addProperty("condition", condition);
        b.addProperty("distractor", distractor);
        b.addProperty(fc_1_selection,1);
        b.addProperty(fc_1_unselected,0);
        b.addProperty(fc_2_selection,1);
        b.addProperty(fc_2_unselected,0);
        b.addProperty(fc_3_selection,1);
        b.addProperty(fc_3_unselected,0);
        b.addProperty(fc_4_selection,1);
        b.addProperty(fc_4_unselected,0);
        return b;
    }
    ...
    public static void main(String[] args){
        ...
        for (JSONParticipant p : data.getParticipants()){
            if((Integer.parseInt(p.attention1) == 1) & (Integer.parseInt(p.attention2) == 1) && (p.shape.toLowerCase().contains("shape"))){
                participantFile.add("participant " + count, p.forcedChoice()); }
            count+=1;
        }
        Gson gson = new Gson();
        try {
            Writer writer = Files.newBufferedWriter(Paths.get("/Users/rgelpi/Documents/Explore-Exploit/JSON/fc_blocks.json"));
