...
public class JSONData {
    ...
    public void printBlicketsToCsv(String path){
        Path output = Paths.get(path);
        List<String> lines = new ArrayList<>();
        lines.add("FC1 Correct?,FC1 First Blicket Consistent?,FC1 Second Blicket Consistent?,FC2 Correct?,FC2 First Blicket Consistent?,FC2 Second Blicket Consistent?,FC3 Correct?,FC3 First Blicket Consistent?,FC3 Second Blicket Consistent?,FC4 Correct?,FC4 First Blicket Consistent?,FC4 Second Blicket Consistent?,R1 Correct?,R1 Consistent?,R2 Correct?,R2 Consistent?,R3 Correct?,R3 Consistent?,R4 Correct?,R4 Consistent?,R5 Correct?,R5 Consistent?,R6 Correct?,R6 Consistent?,R7 Correct?,R7 Consistent?,R8 Correct?,R8 Consistent?");
        for (JSONParticipant p : this.getParticipants()) {
            lines.add(p.blicketOutput());
        }
        try { Files.write(output,lines, Charset.forName("UTF-8")); }
        catch (IOException e) { e.printStackTrace(); }
    }
    ...
    public static void main(String[] args){
        ...
        data.printToCsv("/Users/rgelpi1/Documents/2019/Explore-Exploit/test2.csv");
        data.printBlicketsToCsv("/Users/rgelpi1/Documents/2019/Explore-Exploit/blickets2.csv");
    }
}
